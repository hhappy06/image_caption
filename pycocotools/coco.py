...
class COCO:
    ...
    def filter_by_cap_len(self, max_cap_len):
        print("Removing extremely long captions... more than %d"%(max_cap_len))
        keep_ann = {}
        keep_img = {}
        for ann in self.dataset['annotations']:
    ...
    def all_captions(self):
        return [ann['caption'] for ann in self.dataset['annotations']]

    def filter_by_words(self, wordset):
        print("Removing excluded words...")
        keep_ann = {}
        keep_img = {}
        for ann in self.dataset['annotations']:
            if self._check_annotations_satify_wordset(ann, wordset):
                keep_ann[ann['id']] = keep_ann.get(ann['id'], 0) + 1
                keep_img[ann['image_id']] = keep_img.get(ann['image_id'], 0) + 1
    ...
    def _check_annotations_satify_wordset(self, ann, wordset):
        if not ann:
            return False
